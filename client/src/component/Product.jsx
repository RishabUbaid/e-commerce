...
function Product({ curElem }) {
  return (
    <div className="food-card bg-gray-100 rounded-xl flex flex-col cursor-pointer items-center p-5">
      <div className="relative mb-3">
        <Link to={`/products/${curElem?._id}`}>
          <img src={curElem?.productImage} alt="" />
        </Link>
        <div className='absolute top-2 left-2'>
          <button className='shadow-sm text-white bg-gray-300 hover:bg-gray-400 cursor-pointer p-5 rounded-full relative'>
      ...
      <button className='bg-[#FDC55E] active:scale-90 transition duration-150 transform hover:shadow-xl shadow-md rounded-full px-8 py-2 text-xl font-medium text-white'>
        Buy now
      </button>
    </div>
  );
