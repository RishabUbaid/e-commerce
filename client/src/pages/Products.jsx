      ...
      <div className='container mx-auto py-8'>
        <div className='p-5 mb-14'>
          <div className='flex flex-wrap justify-center mb-8 gap-5'>
            {category?.map((btn) => (
              <button
                className={
                  active === btn.id
                    ? 'text-xl px-4 py-3 text-center text-white bg-gray-300 border-gray-300 border-2 rounded-sm justify-center font-medium'
                    : 'text-xl px-4 py-3 text-gray-300 border-gray-300 border-2 font-medium'
                }
                onClick={() => {
                  handleBtn(btn);
                }}
          ...
          <div className='grid py-6  gap-8 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1'>
            {product?.map((curElem) => (
              <div className='food-card bg-gray-300/10 rounded-xl flex flex-col cursor-pointer items-center p-5' key={curElem?._id}>
                <div className='relative mb-3'>
                  <Link to={`/products/${curElem?._id}`}>
                    <img src={curElem?.productImage} alt='' />
                  </Link>
                  <div className='absolute top-2 left-2'>
                    <button className='shadow-sm text-white bg-gray-300 hover:bg-gray-500 cursor-pointer p-5 rounded-full relative'>
                ...
                <div className='flex gap-4 items-center'>
                  <p className='text-xl text-center font-bold text-green-300'>{curElem?.name}</p>
                ...
                <button
                  className='bg-green-300 active:scale-90 transition duration-150 transform hover:shadow-xl shadow-md rounded-full px-8 py-2 text-xl font-medium text-white'
